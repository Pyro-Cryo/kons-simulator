...
export class BasicAnimation {
  ...
  _completeConstruction() {
    const properties = this.keyframes.reduce((map, frame) => {
      for (const property of frame[0].keys()) {
        ...
        if (map.has(property)) map.set(property, isNumber && map.get(property));
        else map.set(property, isNumber);
      }
      return map;
    }, new Map());
    ...
    properties.forEach((isNumber, property) => {
      const map = isNumber
        ? this.interpolatedProperties
        : this.uninterpolatedProperties;
      const relevantFrames = this.keyframes.filter((frame) =>
        frame[0].has(property)
      );
      map.set(property, {
    ...
    this.totalTime = Math.max(
      ...this.keyframes.map((frame) => frame[1]),
      ...this.funcs.map((frame) => frame[1])
    );
  ...
  stay(time) {
    ...
    if (time < 0) throw new Error(`Invalid time between frames: ${time}`);
  ...
  loop() {
    ...
    if (this.keyframes.length !== 0) this.set(this.keyframes[0][0]);
    else this.call(null);
  ...
  clone() {
    ...
    if (this.constructionComplete) {
      ...
      animation.uninterpolatedProperties =
        this.uninterpolatedProperties.slice();
  ...
  start(obj = null) {
    if (!this.constructionComplete)
      throw new Error(
        'Cannot start animation before it has been completed with loop() ' +
          'or done()'
      );
    ...
    this.obj.update = (delta) => {
      ...
      for (const [func, timeToRun] of this.funcs) {
        if (func && this.time < timeToRun && timeToRun <= timeNext) func();
      }
