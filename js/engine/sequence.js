...
class BaseSequence {
  ...
  _checks(requireNothingQueued) {
    if (
      requireNothingQueued !== undefined &&
      requireNothingQueued ^ (this.currentSequence.length === 0)
    ) {
      console.log(this);
  ...
  get length() {
    return this.totalSequence
      ...
      .reduce((tot, ins) => (ins[0] === 'wait' ? tot + ins[1] : tot), 0);
  }
  ...
  wait(delay) {
    this._checks(true);
    if (!(delay >= 0)) throw new Error('Invalid delay ' + delay);
  ...
  spawn(type, number = 1) {
    this._checks();
    if (number < 0) throw new Error('Invalid number ' + number);
    this.currentSequence = this.currentSequence.concat(
      new Array(number).fill(['spawn', type])
    );
   ...
   * @param {Number} interval Längden på intervallet i enheter av
   * 		rum/tids-avstånd
   * @param {boolean} integerize Om avstånden mellan två objekt alltid ska vara
   * 		heltal.
   */
  over(interval, integerize = false) {
    ...
    } else delays = delays.fill(interval / nDelays);

    for (let i = 0; i < this.currentSequence.length; i++) {
      if (i !== 0) this.totalSequence.push(['wait', delays[i - 1]]);
  ...
  spaced(interval) {
    ...
    for (let i = 0; i < this.currentSequence.length; i++) {
      if (i !== 0) this.totalSequence.push(['wait', interval]);
      this.totalSequence.push(this.currentSequence[i]);
    }
    this.currentSequence = [];
   ...
   * @param {Number} times Antal upprepningar. Om denna sätts till 0 (default)
   * 		anger det funktionsanropet direkt (motsvarar ungefär
   * 		`call(func, 1).immediately()`).
   */
  call(func, times = 0) {
    if (!(times >= 0)) throw new Error('Invalid times ' + times);
    else if (times === 0) {
      this._checks(true);
      ...
      this._checks();
      this.currentSequence = this.currentSequence.concat(
        new Array(times).fill(['call', func])
      );
    }
  ...
  interleave(other) {
    ...
    while (
      this_ind < this.totalSequence.length &&
      other_ind < other.totalSequence.length
    ) {
    ...
    if (this_ind !== this.totalSequence.length)
      this.currentSequence = this.currentSequence.concat(
        this.totalSequence.slice(this_ind)
      );
    if (other_ind !== other.totalSequence.length)
      this.currentSequence = this.currentSequence.concat(
        other.totalSequence.slice(other_ind)
      );
   ...
   * @param {["call", *]} instruction En array med "call" först, följt av
   ...
   * @param {[string, ...*]} instruction En array med instruktionstypen först,
   * följt av eventuella argument.
   */
   ...
   */
  next(delta = 1) {
    // Börja iterera första gången next() anropas, och förhindra att sekvensen
    // ändras.
    if (!this.iterating) {
      this.iterating = true;
      this.index = 0;
    ...
    while (delta !== null) {
      ...
      while (
        this.index < this.totalSequence.length &&
        (this.totalSequence[this.index][0] !== 'wait' ||
      ...
      } else return {done: true, remainingDelta: delta};
    }
  ...
  cloneInstruction(instruction) {
    if (instruction[0] === 'wait') return ['wait', instruction[1]];
...
class ArgableSequence extends BaseSequence {
   ...
   *
   * Argumenten kan vara antingen en array som populerar parametrarna till
   * konstruktorn, eller en funktion som anropas vid spawnandet och då
   * returnerar en sådan array.
   * @param {*} type Objektens typ.
   * @param {Number} number Antalet objekt som ska köas.
   * @param {Function | Array} args Argumenten till konstruktorn för typen,
   * 		eller en funktion som returnerar dessa.
   */
   ...
   * @param {["spawn", *] | ["spawn", *, Array | Function]} instruction En array
   * 		med "spawn" först, följt av typen som ska skapas, följt av eventuella
   * 		argument.
   */
  doSpawn(instruction) {
    if (instruction.length > 2 && instruction[2] !== null) {
      let args = instruction[2];
      if (args instanceof Function) args = args(this.elapsed);
      return new instruction[1](...args);
    } else return new instruction[1]();
  }
   ...
   * 		anger det funktionsanropet direkt (motsvarar ungefär
   * 		`call(func, 1).immediately()`).
   * @param {Function | Array} args Argumenten till funktionen, eller en
   * 		funktion som i sig returnerar dessa.
   */
  ...
  /**
   * Anropa en funktion
   * @param {["call", *] | ["call", *, Array | Function]} instruction En array
   * 		med "call" först, följt av funktionen som ska anropas, följt av
   * 		eventuella argument.
   */
  doCall(instruction) {
    if (instruction.length > 2 && instruction[2] !== null) {
