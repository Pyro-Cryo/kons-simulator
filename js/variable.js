...
class FixedModifier extends Modifier {
    ...
    canBeRemoved() {
        return this.duration !== null && Clock.now() - this.createdAt >= this.duration;
    }
}
...
class ManualModifier extends FunctionalModifier {
    ...
    set progress(value) {
        if (value < this._progress) {
            throw new Error(`Progress cannot be lowered, got: ${value} < ${this._progress}`)
        }
...
class MonitoringModifier extends Modifier {
    ...
    constructor(variables, func, description = null) {
        const descriptionFunc = typeof description === 'function' ? description : null;
        super(descriptionFunc === null ? description : null);
        this.func = func;
        this.variables = variables instanceof BaseVariable ? [variables] : variables;
     ...
     * @param {number} min The value of the modifier when the variable is at its minimum.
     * @param {number} max The value of the modifier when the variable is at its maximum.
    ...
    static linearMapOnto(variable, min, max, ...descriptions) {
        ...
        return new MonitoringModifier(
            variable, value => offset + multiplier * value, description
        );
    }
    ...
    /** @param {...BaseVariable} terms */
     static sumOf(...terms) {
        ...
        return new MonitoringModifier(
            terms, (...values) => values.reduce((a, b) => a + b)
        );
    }
...
class BaseVariable {
    ...
    /** @returns {number} */
    static get min() { return -Infinity; }
    /** @returns {number} */
    static get max() { return Infinity; }
    /** @returns {string} */
     ...
     * @param {string} unit 
     */
    constructor(title = null, description = null, min = null, max = null, unit = '') {
        ...
        if (this.min >= this.max) {
            throw new Error(`Min must be less than max, got: ${this.min} >= ${this.max}`);
        }
    }
...
class Variable extends BaseVariable {
    ...
    constructor(title = null, description = null, baseValue = null, min = null, max = null, unit = null) {
...
class FurnaceVariable extends BaseVariable {
     ...
     * @param {number} duration The number of game minutes until the fuel is consumed.
     * @param {string | null} description 
     * @param {number | null} weight The priority for this modifier. Smaller weights are consumed first. Defaults to the rate at which the value diminishes.
     */
    addFuel(quantity, duration, description = null, weight = null) {
        ...
        this._modifiers.push(
            new ManualModifier(
                t => quantity - t * burnRate, duration, description
            ),
            weight ?? -burnRate,
        );
    }
    ...
    getValue() {
        ...
        while (timeToProgress > 0 && !this._modifiers.isEmpty()) {
            ...
            if (currentModifier.duration - currentModifier.progress > timeToProgress) {
