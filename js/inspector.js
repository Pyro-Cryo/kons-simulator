...
export class Inspector extends GameObject {
  constructor() {
    ...
    this.variableTemplate = this.variables
      .getElementsByClassName('template')
      .item(0);
  ...
  static _summarizeModifiers(variable) {
    ...
    return Array.from(
      ...
      (entry) =>
        `${entry[0]} ${entry[1].count > 1 ? `x${entry[1].count} ` : ''}(${
          entry[1].sum > 0 ? '+' : ''
        }${variable.formatValue(entry[1].sum)})`
    );
  }
  ...
  _monitorVariables(variables) {
    ...
    for (const variable of variables) {
      ...
      variableRoot.getElementsByClassName('title').item(0).innerText =
        variable.title;
  ...
  updateStaticItemInfo(item) {
    ...
    const variables = [item.maybeGetInterface(Heatable)?.temperature].filter(
      (v) => !!v
    );
